...
def download_task_plots(task_id: str, output_dir: Path, max_plots: int = 5) -> List[str]:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Found {len(scalar_metrics) if scalar_metrics else 0} scalar metric groups for task {task_id}"
            )
            ...
            if scalar_metrics:
                ...
                if metrics_data:
                    ...
                    ax2.set_xticklabels(
                        [name.replace("_", " ").title() for name in metric_names],
                        rotation=45,
                        ha="right",
                    )
                    ...
                    # Save combined plot
                    plot_filename = "combined_metrics.png"
                                    ...
                                    if artifact_path.exists():
                                        ...
                                        if artifact_path.suffix.lower() == ".csv":
                                            ...
                                            if cm_path.exists():
                                                ...
                                                logger.success(
                                                    f"Created confusion matrix from CSV: {cm_path}"
                                                )
                                        elif (
                                            str(artifact_path)
                                            .lower()
                                            ...
                                            if cm_path.exists():
                                                ...
                                                confusion_matrix_found = True
                                                logger.success(
                                                    f"Copied confusion matrix image: {cm_path}"
                                                )
                            except Exception as e_art:
                                logger.debug(
                                    f"Could not process confusion matrix artifact {artifact_name}: {e_art}"
                                )
...
def generate_experiment_report(
    ...
    try:
        ...
        if tasks:
            ...
            logger.warning(
                f"No tasks found in project '{project_name}'. Trying alternative project names..."
            )
    ...
    if not project_name:
        ...
        raise ValueError(
            "No ClearML project with experiments found. Please ensure experiments are run first."
        )
...
"""
    ...
    for metric_name in metrics:
        ...
        try:
            ...
            for result in results:
                ...
                if task_id and task_id not in seen_task_ids:
                    if task_name and task_name in seen_task_names:
                        logger.debug(
                            f"Skipping duplicate task name: {task_name} (task_id: {task_id})"
                        )
                        continue
            ...
            logger.info(
                f"Found {len(table_results)} unique experiments for table (from {len(unique_results)} total)"
            )

            # If no results from experiments, try model registry as fallback
            if not table_results:
                logger.info("No experiments found, trying model registry...")
    ...
    if include_plots and experiments_by_metric:
        ...
        for task_id, task_name, metric_name in selected_experiments[
            :4
        ]:  # Limit to 4 different experiments

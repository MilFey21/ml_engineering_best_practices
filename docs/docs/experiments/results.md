...
**Примечание:** Для сравнения экспериментов рекомендуется использовать ClearML Web UI, который предоставляет более богатые возможности визуализации и анализа. Код использует ClearML API напрямую без дополнительных оберток для упрощения и прозрачности.

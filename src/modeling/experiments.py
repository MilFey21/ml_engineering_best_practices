...
def train_and_log_experiment(
    ...
    # Initialize ClearML task directly
    task = Task.init(
    ...
    try:
        ...
        # Log hyperparameters directly
        task.connect(all_params)
        ...
        if n_iterations and supports_warm_start:
            ...
            logger_instance.report_scalar("Metrics", "train_accuracy", train_acc, iteration=0)
            logger_instance.report_scalar("Metrics", "test_accuracy", test_acc, iteration=0)
            logger_instance.report_scalar("Metrics", "test_f1_score", test_f1, iteration=0)
        ...
        cm_path = MODELS_DIR / f"confusion_matrix_{model_type}_{task.id}.csv"
        cm_df.to_csv(cm_path, index=True)
        task.upload_artifact(name="confusion_matrix", artifact_object=str(cm_path))

        # Log confusion matrix using ClearML's built-in method
        try:
            logger_instance.report_confusion_matrix(
                title="Confusion Matrix",
                series=f"Confusion Matrix - {exp_task_name}",
                matrix=cm.tolist(),
            ...
            # Fallback: log as image if report_confusion_matrix is not available
            ...
            ax.set_title(f"Confusion Matrix - {exp_task_name}")
        ...
        if hasattr(model, "feature_importances_"):
            ...
            importance_path = MODELS_DIR / f"feature_importance_{model_type}_{task.id}.csv"
            feature_importance.to_csv(importance_path, index=False)
            task.upload_artifact(name="feature_importance", artifact_object=str(importance_path))

        # Save and upload model
        model_path = MODELS_DIR / f"model_{model_type}_{task.id}.pkl"
...
def run_experiments(
    ...
    if not results_df.empty:
        ...
        # Note: Experiment comparison can be done directly in ClearML Web UI
        logger.info("\n" + "=" * 60)
        logger.info("View experiments in ClearML Web UI for comparison")
        logger.info("=" * 60)
